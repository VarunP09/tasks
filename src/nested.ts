...
export function toCSV(questions: Question[]): string {
    return questions.reduce((collective: string, question:Question): string => collective += question.id+","+question.name+","+question.options.length+","+question.points+","+question.published+"\n" , "id,name,options,points,published\n").trim();
}
...
export function publishAll(questions: Question[]): Question[] {
    return questions.map((question: Question): Question => ({...question, published: true, options: [...question.options]}));
}
...
export function sameType(questions: Question[]): boolean {
    ...
    return questions.filter((question: Question): boolean => question.type === firstType).length === questions.length;
}
...
export function editOption(
    ...
    function putInValue(arr: string[], index: number, elem: string): string[]{
        if(index === -1) return [...arr, elem];
        let copy: string[] = [...arr];
        copy.splice(index, 1, elem);
        return copy;
    }
    return questions.map((question: Question): Question => question.id === targetId ? {...question, options: putInValue(question.options, targetOptionIndex, newOption) } : question);
}
...
export function duplicateQuestionInArray(
    ...
    return questions.reduce((accumulate: Question[], question: Question): Question[] => question.id === targetId ? accumulate = [...accumulate, question, duplicateQuestion(newId, question)] : accumulate = [...accumulate, question], []);
}
